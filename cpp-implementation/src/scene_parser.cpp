...
void parse_viewport(std::istringstream& ss, scene& s)
{
    ...
    {
        std::println(std::cerr,
                     "bad viewport (expected: viewport width height)");
        return;
    }
...
void parse_camera(std::istringstream& ss, scene& s)
{
    ...
    std::println("parsed camera pos=({}, {}, {}) look_at=({}, {}, {}) "
                 "up=({}, {}, {}) near={} far={}",
                 pos.x, pos.y, pos.z, look_at.x, look_at.y, look_at.z, up.x,
                 up.y, up.z, near_plane, far_plane);
}
...
void parse_fov(std::istringstream& ss, scene& s)
{
    ...
    std::println("parsed fov degrees={} radians={}", degrees,
                 s.camera_data.fov);
}
