...
std::optional<cg::hitevent> get_ray_mesh_hit(
    const mesh3d& mesh, std::span<const triangle> triangles,
    std::span<const vertex> vertices, ray r, interval valid)
{
    ...
    const auto handle_leaf = [&](const bvh_node& node) {
        ...
        for (std::uint32_t i = 0; i < node.triangle_count; ++i)
        {
            ...
            if (auto hit = get_ray_triangle_hit(tri, vertices, r, valid, false))
            {
                valid.max =
                    hit->t; // narrow the interval — discard farther hits
                result = hit;
            }
        }
    ...
    const auto handle_node = [&](const bvh_node& node) {
        ...
        const auto aabb_r = aabb{mesh.blas.nodes[right].box_min,
                                 mesh.blas.nodes[right].box_max};
